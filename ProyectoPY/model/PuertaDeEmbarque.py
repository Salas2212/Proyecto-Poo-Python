...
class PuertaDeEmbarque:
    ...
    def imprimir_info(self):
        ret = ''
        ret += f"id: {self.identificacion} | ubicacion: {self.ubicacion} | disponible: {'Si' if self.disponible else 'No'}"
        if not self.disponible:
            ret += f"  Id vuelo: {self.vuelo_asignado.numero_de_identificacion} | hora embarque: {self.hora_embarque}"
        #print (ret)
        return ret
    ...
    def imprimir_historial(self):
        ...
        for i, vuelo in enumerate(self.historial, start=1):
            ret += f"{i}. {vuelo.numero_de_identificacion} | {vuelo.ciudad_origen} - {vuelo.ciudad_destino}. Fecha: {vuelo.fecha}\n"
        return ret
